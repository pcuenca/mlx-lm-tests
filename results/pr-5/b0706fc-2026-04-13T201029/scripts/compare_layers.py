...
def collect_transformers_states(model_path, input_ids):
    ...
    states = [h[0].numpy() for h in outputs.hidden_states]
...
def collect_mlx_states(model_path, input_ids):
    """Return a list of numpy arrays: [embedding_output, layer_0_output, ...]."""
    ...
    return [np.array(h[0].astype(mx.float32)) for h in captured]
...
def main():
    ...
    tf_states = collect_transformers_states(model_path, input_ids)
    mlx_states = collect_mlx_states(model_path, input_ids)
