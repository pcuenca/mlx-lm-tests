...
- **Timestamp:** 2026-04-14T10:33:12+00:00

## Numerical comparison (transformers vs MLX)

| Variant | Max diff | Mean diff | <0.01 | <0.1 | Top-1 | Top-5 | Top-10 |
|---|---|---|---|---|---|---|---|
| [`allenai/Olmo-Hybrid-7B`](#allenaiolmo-hybrid-7b) | 0.506462 | 0.073131 | 8.75% | 73.77% | True | 5/5 | 10/10 |
| [`allenai/Olmo-Hybrid-Instruct-SFT-7B`](#allenaiolmo-hybrid-instruct-sft-7b) | 0.255213 | 0.048441 | 12.98% | 90.18% | True | 5/5 | 10/10 |
| [`allenai/Olmo-Hybrid-Instruct-DPO-7B`](#allenaiolmo-hybrid-instruct-dpo-7b) | 0.553753 | 0.045662 | 13.66% | 93.48% | True | 5/5 | 10/10 |
| [`allenai/Olmo-Hybrid-Think-SFT-7B`](#allenaiolmo-hybrid-think-sft-7b) | 0.272760 | 0.042059 | 15.88% | 93.47% | True | 5/5 | 10/10 |

## Layer-by-layer comparison

| Variant | Worst layer | Diverged | Post-norm | Logits rel | Logits max diff |
|---|---|---|---|---|---|
| [`allenai/Olmo-Hybrid-7B`](#allenaiolmo-hybrid-7b) | layer 30 (2.01%) | 0 | 2.61% | 1.33% | 0.5000 |
| [`allenai/Olmo-Hybrid-Instruct-SFT-7B`](#allenaiolmo-hybrid-instruct-sft-7b) | layer 2 (2.33%) | 0 | 2.60% | 1.05% | 0.3750 |
| [`allenai/Olmo-Hybrid-Instruct-DPO-7B`](#allenaiolmo-hybrid-instruct-dpo-7b) | layer 30 (2.00%) | 0 | 3.19% | 1.69% | 0.6250 |
| [`allenai/Olmo-Hybrid-Think-SFT-7B`](#allenaiolmo-hybrid-think-sft-7b) | layer 12 (2.03%) | 0 | 2.34% | 1.15% | 0.3750 |
...
==========
Prompt: 57 tokens, 459.297 tokens-per-sec
Generation: 200 tokens, 42.943 tokens-per-sec
...
==========
Prompt: 26 tokens, 221.028 tokens-per-sec
Generation: 4096 tokens, 41.926 tokens-per-sec
...
==========
Prompt: 57 tokens, 564.841 tokens-per-sec
Generation: 200 tokens, 104.292 tokens-per-sec
...
==========
Prompt: 51 tokens, 416.217 tokens-per-sec
Generation: 77 tokens, 43.141 tokens-per-sec
...
==========
Prompt: 49 tokens, 401.844 tokens-per-sec
Generation: 518 tokens, 42.594 tokens-per-sec
Peak memory: 15.033 GB
````
</details>
...
==========
Prompt: 51 tokens, 509.457 tokens-per-sec
Generation: 143 tokens, 105.035 tokens-per-sec
...
## **3
==========
Prompt: 51 tokens, 408.196 tokens-per-sec
Generation: 200 tokens, 42.942 tokens-per-sec
...
==========
Prompt: 49 tokens, 405.517 tokens-per-sec
Generation: 3482 tokens, 42.155 tokens-per-sec
Peak memory: 15.420 GB
````
</details>
...
````text
==========
Certainly! A **Transformer neural network** (like those in BERT, GPT, or T5) is a state-of-the-art architecture for understanding and generating language. Here’s a **step-by-step explanation** of how a Transformer processes a sentence:

---

## **1. Tokenization**
- **Input sentence** (e.g., *"The cat sat on the mat."*) is split into **tokens** (subwords or words).
- **Special tokens** are added:
  - `[CLS]` (for classification tasks)
  - `[SEP]` (to separate sentences in a pair)
  - Padding tokens if lengths differ.
- **Example tokens:** `[CLS] the cat [SEP] sat on the mat [SEP]`

---

## **2. Embedding**
- Each token is mapped to a **vector** (embedding) using a **token embedding matrix**.
...
==========
Prompt: 51 tokens, 509.714 tokens-per-sec
Generation: 200 tokens, 105.515 tokens-per-sec
...
==========
Prompt: 54 tokens, 431.750 tokens-per-sec
Generation: 200 tokens, 42.739 tokens-per-sec
...
==========
Prompt: 52 tokens, 418.285 tokens-per-sec
Generation: 4096 tokens, 41.971 tokens-per-sec
...
````text
==========
Okay, so I need to explain how a transformer neural network processes a sentence step by step. Hmm, where do I start? I remember that transformers are different from RNNs or LSTMs because they don't use recurrence. Instead, they use self-attention mechanisms. Let me think about the steps involved.

First, the input sentence is probably converted into tokens. Each word or subword is a token. Then, each token is converted into a vector, right? So the first step is tokenization. Then, each token is embedded into a vector, maybe using an embedding matrix. So the first step is tokenization and embedding.

Next, there's the positional encoding. Since transformers don't have inherent information about the order of tokens, they add positional encodings to the embeddings. These encodings are added to the token embeddings. The positional encodings are sinusoidal functions, right? So for each position, they create a vector with sine and cosine functions of different frequencies. That
==========
Prompt: 54 tokens, 528.412 tokens-per-sec
Generation: 200 tokens, 104.234 tokens-per-sec
Peak memory: 4.434 GB
````
</details>
